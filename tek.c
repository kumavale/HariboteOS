...
#include <setjmp.h>
#include <bits/types/__sigset_t.h>  /* Get '__sigset_t' */
...
int tek_decomp(unsigned char *p, char *q, int size)
{
    int err = -1;
    if (*p == 0x83) {
        err = tek_decode1(size, p, (UCHAR *) q);
    } else if (*p == 0x85) {
        err = tek_decode2(size, p, (UCHAR *) q);
    } else if (*p == 0x89) {
        err = tek_decode5(size, p, (UCHAR *) q);
    }
    if (err != 0) {
        return -1;
    }
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags)
{
    ...
    *q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
    ...
    while (pos < osiz) {
        s_pos = pos & m_pos;
        if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^ stk) { /* not lz */
